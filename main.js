...
function main() {
  ...
  window.onkeydown = (event) => {
    if (event.key == 'w') controller.w = true;
    if (event.key == 'a') controller.a = true;
    if (event.key == 's') controller.s = true;
    if (event.key == 'd') controller.d = true;
  };

  window.onkeyup = (event) => {
    if (event.key == 'w') controller.w = false;
    if (event.key == 'a') controller.a = false;
    if (event.key == 's') controller.s = false;
    if (event.key == 'd') controller.d = false;
  };
  ...
  function render() {
    ...
    if (!freeze) {
      ...
      theta =
        theta > 2.0 * Math.PI
          ? 0.0
          : curveValue(theta, 0.0, 2.0 * Math.PI, 0.94);
    }

    if (controller.w) translation[1] += 0.01;
    if (controller.a) translation[0] -= 0.01;
    if (controller.s) translation[1] -= 0.01;
    if (controller.d) translation[0] += 0.01;
...
function curveValue(x, min, max, exponential) {
  ...
  return (
    min +
    (max - min) *
      (Math.pow(val - min, exponential) / Math.pow(max - min, exponential))
  );
}
